...
# Outputs: /tmp/superlite-files-musl/{usr/bin/superlite-files, usr/lib/superlite-files/...}
#
...
if [ ! -d "$SRC" ]; then
    log "ERROR: SuperLite Files source not found at $SRC"
    exit 1
fi
...
if [ -z "$TAURI_BIN" ]; then
    TAURI_BIN=$(find src-tauri/target -name "superlite-files" -type f 2>/dev/null | head -1)
fi

if [ -z "$TAURI_BIN" ] || [ ! -f "$TAURI_BIN" ]; then
    log "ERROR: superlite-files binary not found"
    find src-tauri/target -name "superlite-files*" -type f 2>/dev/null | head -10
    exit 1
fi
...
rm -rf "$OUTPUT"
mkdir -p "$OUTPUT/usr/bin" "$OUTPUT/usr/lib/superlite-files"
cp -v "$TAURI_BIN" "$OUTPUT/usr/bin/superlite-files"
chmod +x "$OUTPUT/usr/bin/superlite-files"

for so in src-tauri/target/release/*.so src-tauri/target/release/deps/*.so; do
    [ -f "$so" ] && cp -v "$so" "$OUTPUT/usr/lib/superlite-files/" 2>/dev/null || true
done

log "=== Done ==="
ls -la "$OUTPUT/usr/bin/superlite-files"
file "$OUTPUT/usr/bin/superlite-files"
